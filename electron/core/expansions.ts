...
export const expansionsPostMessage = (
  ...
  if (child && child.connected) {
    ...
    } else if (data.type === 'webview-get-expansions') {
      // 是 webview的消息要保存窗口
      const __name = data.data.name
      webviewWindows.set(__name, webContents)
    }
...
// const webviewTypes = ['webview-on-message', 'webview-get-expansions']
...
//     if (webContents.isDestroyed()) return
//     try {
//       if (webviewTypes.includes(message.type)) {
//         const __name = message.data.name
...
//           const content = webviewWindows.get(__name)
//           if (!content) return
//           content?.send(webviewTypesMap[message.type], message.data)
//           return
//         }
//       } else {
//         webContents.send('expansions-message', message)
...
//       webviewWindows.set(__name, webContents)
//     }
//     if (webviewTypes.includes(data.type)) {
//       save()
//     }
//     child.send(data)
//     return true
